...
export function jerseySvg(number, variant = "home", surname = "") {
  const back = Boolean(surname);
  const name = String(surname)
    .toUpperCase()
    .replace(
      /[&<>"']/g,
      (c) =>
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 280" role="img" aria-labelledby="title" data-number="${value}" data-view="${back ? "back" : "front"}">
${home || back ? "" : `<style>@font-face{font-family:UniformScript;src:url(data:font/woff2;base64,${scriptFont}) format("woff2");font-weight:400}</style>`}
<title id="title">Reference-based ${home ? "black home" : "white away"} ${back ? "rear-view jersey illustration, " + name : "jersey illustration"}, number ${value}</title>
...
<path fill="${home ? "#080808" : "#fff"}" stroke="${home ? "#333" : "#111"}" stroke-width="5" d="${back ? "M111 40 Q150 62 189 40 L179 51 Q150 66 121 51Z" : "m111 40 39 22 39-22-10 27-29 11-29-11Z"}"/>
<path fill="none" stroke="${home ? "#333" : "#ddd"}" stroke-width="2" d="m101 111-7 128m105-128 7 128M88 249h126"/>
${back ? `<text x="150" y="103" text-anchor="middle" fill="${ink}" font-family="Arial, sans-serif" font-size="20" font-weight="700" textLength="120" lengthAdjust="spacingAndGlyphs">${name}</text>` : `<text x="150" y="105" text-anchor="middle" fill="${ink}" font-family="${home ? "Arial, sans-serif" : "UniformScript, cursive"}" font-size="${home ? "15" : "28"}" font-weight="${home ? "700" : "400"}" ${home ? 'textLength="100" lengthAdjust="spacingAndGlyphs"' : ""}>${home ? "BUCKAROOS" : "Smackover"}</text>`}
